...
def tournaments_input(input_):
    ...
    all_tournaments = ['Стыки', 'ОПК', '8x8', 'ЧВ', 'КР', 'МКР', 'ЛП', 'КП', 'ЗЛ']
    tournaments = []
...
def tournament_to_caption(t):
    ...
    captions = {'опк': 'Чемпионат ОПК', 'стыки': 'Чемпионат ОПК', '8x8': 'Лига 8x8', 'кр': 'Кубок Ректора', 'мкр': 'Малый Кубок Ректора',
...
def subtournament_to_str(t, s):
    ...
    if t.lower().strip() in ['опк', '8x8', 'чв', 'лп', 'зл']:
...
def tournament_to_cover_text(t, s):
    ...
    tournaments = {'опк': 'Чемпионат ОПК', 'стыки': 'Чемпионат ОПК', '8x8': 'Лига 8x8', 'кр': 'Кубок Ректора', 'мкр': 'Малый Кубок Ректора',
                   'чв': 'Чемпионат Выпускников', 'лп': 'Летнее Первенство', 'кп': 'Кубок Первокурсника', 'зл': 'Зимняя Лига'}
    t = t.split()
    ...
    line2 = ''
    if len(t) == 3:
        if th in ['опк', '8x8', 'чв', 'лп', 'зл', 'кр', 'мкр', 'кп']:
